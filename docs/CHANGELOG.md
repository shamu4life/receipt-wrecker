...
- **`Script` and `Fantasy` were duplicates of fonts already in the menu.** Field-confirmed on the real printer: Script came out as **Comic Sans MS**, Fantasy as **Impact** — the standard Windows mappings of the bare CSS generics `cursive` and `fantasy`. A nine-font menu was really seven, and no bench on macOS could have caught it, because macOS maps those two generics somewhere else entirely. They now name faces that exist on Windows with the generic kept behind as a fallback (`Segoe Script,cursive`, `Papyrus,fantasy`), so neither can be worse than it was. Script costs +35 characters instead of +22, Papyrus +30; the card hint says 20–35. `Serif` and `Monospace` stay bare deliberately — Windows maps them to Times New Roman and Courier New, which duplicate nothing.
