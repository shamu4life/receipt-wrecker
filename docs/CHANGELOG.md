...
- `i.uwutoowo.com` is declared but **not yet used**: no `RW_IMG_HOST` var, so `/upload` keeps minting links on the request origin. Before this change that hostname resolved to Cloudflare but was bound to nothing and returned 522 — switching the var on before the domain answers would make every picture payload print blank, so the two steps are deliberately separate commits.
