...
What it reproduces, from printer-bot's own shipped files:
  ...
  * the paper — 203dpi, hard-thresholded to 1 bit, which is what the thermal head does.
...
def ink(png, thresh=128):
    """Hard-threshold to 1 bit and count black pixels — what the head lays down.
...
def main():
    if len(sys.argv) < 2:
        sys.exit("usage: <payload html on stdin> | python3 tools/rig.py <case-name> [avatar-attr]")
    stem = sys.argv[1]
    avatar_attr = sys.argv[2] if len(sys.argv) > 2 else None
    out = render(sys.stdin.read(), stem, avatar_attr)
    if "error" in out:
